#!/usr/bin/env python3
"""Parked-only, single-shot ARS408 Sensor ID 7 -> 3 NVM migration."""
...
SUCCESS_MARKER = LOG_DIR / "ars408_id3_migration_complete.json"
CONFIRMATION = "MIGRATE_ID7_TO_ID3_ONCE"
...
def main() -> int:
  ...
  current_raw = wait_for_frame(sock, CURRENT_STATE_ADDRESS, 8, 4.0)
  if current_raw is None:
    raise RuntimeError("Refusing migration: current Sensor ID 7 RadarState not observed")
  current_state = decode_state(current_raw)
  if int(current_state["RadarState_SensorID"]) != CURRENT_SENSOR_ID:
    raise RuntimeError(f"Refusing migration: unexpected current state {current_state}")
  ...
  target_raw = wait_for_frame(sock, TARGET_STATE_ADDRESS, 8, 6.0)
  ...
  target_state = decode_state(target_raw)
