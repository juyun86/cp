#!/usr/bin/env python3
"""Parked-only, one-shot ARS408 object-filter commissioning for CP.

This tool must be run with pandad stopped so it can exclusively claim Panda.
It never changes SensorID or RadarCfg. It caps the object list at 32 entries
and disables every other object pass-through filter.
"""
...
from opendbc.car.tesla.ars408_can import (
  ARS408_BUS, ARS408_FILTER_CONFIG_ADDRESS, ARS408_MAX_OBJECTS, ARS408_SENSOR_ID, ARS408CAN,
)
from panda import Panda


CONFIRMATION = "COMMISSION_ARS408_ID5_CP_FILTERS_ONCE"
LOG_DIR = Path("/data/media/0/ars408_diagnostics")
SUCCESS_MARKER = LOG_DIR / "ars408_cp_filters_complete.json"
...
def verify_dedicated_radar_bus(panda: Panda, duration: float = 3.0) -> tuple[dict, dict]:
  ...
  if radar_state is None:
    raise RuntimeError("Refusing commissioning: Sensor ID 5 RadarState was not observed")

  expected_state = {
    "RadarState_SensorID": 5,
...
def main() -> int:
  ...
  if not args.execute:
    print("DRY RUN: would configure object count 32 and disable object filters 1..14")
    return 0
  ...
  report = {
    "test": "ARS408 CP one-time filter commissioning",
