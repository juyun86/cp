#!/usr/bin/env python3
"""Parked-only, single-shot ARS408 Sensor ID 3 -> 2 NVM migration."""
...
SUCCESS_MARKER = LOG_DIR / "ars408_id2_migration_complete.json"
CONFIRMATION = "MIGRATE_ID3_TO_ID2_ONCE"
...
def wait_for_target_state(sock, timeout: float) -> tuple[bytes, dict] | None:
  """Ignore vehicle 0x221 frames and the radar's brief old-ID transition state."""
  deadline = time.monotonic() + timeout
  while time.monotonic() < deadline:
