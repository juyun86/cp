...
PARKED_IGNITION_CONFIRMATION = "COMMISSION_ARS408_ID0_EXTENDED_32_OBJECTS_PARKED_IGNITION_ON_ONCE"
...
def main() -> int:
  ...
  parser.add_argument("--inspect", action="store_true", help="Read health, RadarState, and bus counts without sending CAN")
  parser.add_argument("--parked-ignition-on", action="store_true",
                      help="Allow a powered radar only after an external P/standstill verification")
  ...
  if args.inspect:
    with Panda(cli=False) as panda:
      ...
      print(json.dumps({"health": panda.health(), "radar_state": radar_state, "bus_counts": bus_counts},
                       indent=2, sort_keys=True))
    return 0

  is_offroad = Path("/data/params/d/IsOffroad").read_text().strip() == "1"
  if not is_offroad and not args.parked_ignition_on:
  ...
  if not args.execute:
    ...
    return 0
  required_confirmation = PARKED_IGNITION_CONFIRMATION if args.parked_ignition_on else CONFIRMATION
  if args.confirm != required_confirmation:
    raise RuntimeError(f"Refusing commissioning: --confirm must equal {required_confirmation}")
  ...
  report = {
    ...
    "executed": False,
    "parked_ignition_on": args.parked_ignition_on,
    "is_offroad_before": is_offroad,
  }

  with Panda(cli=False) as panda:
    health = panda.health()
    ignition_on = bool(health["ignition_line"] or health["ignition_can"])
    if health["controls_allowed"]:
      raise RuntimeError(f"Refusing commissioning: controls_allowed must be false: {health}")
    if ignition_on and not args.parked_ignition_on:
      raise RuntimeError(f"Refusing commissioning: ignition is on without parked override: {health}")
    if args.parked_ignition_on and not ignition_on:
      raise RuntimeError(f"Refusing commissioning: parked ignition-on mode requested but radar power is absent: {health}")
    report["before_health"] = health
    panda.can_clear(0xFFFF)
    radar_state, bus_counts = verify_dedicated_radar_bus(panda)
    ...
    try:
      ...
      extended_already_enabled = (
        int(radar_state["RadarState_SendExtInfoCfg"]) == int(ARS408_SEND_EXTENDED) and
        int(radar_state["RadarState_NVMwriteStatus"]) == 1
      )
      report["extended_already_enabled"] = extended_already_enabled
      if extended_already_enabled:
        # A prior one-shot frame can take longer than the initial verification
        # window to finish its NVM write. Never repeat that write once RadarState
        # confirms both Extended and a successful NVM result.
        report["after_radar_state"] = radar_state
      else:
        panda.can_send(*radar_config)
        report["after_radar_state"] = wait_for_extended_state(panda, timeout=12.0)
